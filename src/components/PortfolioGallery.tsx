      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-20">
        ...
        <div className="text-center mb-16">
          ...
          <motion.h2
            ...
            className="text-3xl sm:text-4xl md:text-5xl font-sans tracking-tight font-light text-slate-900 mb-6 leading-tight"
          >
          ...
          <motion.p
            ...
            className="max-w-2xl mx-auto text-slate-500 font-sans leading-relaxed text-sm sm:text-base"
          >
        ...
        <div className="flex flex-wrap justify-center items-center gap-2 sm:gap-3 mb-10 sm:mb-16">
          {(['all', 'fashion', 'graphic', 'hybrid'] as const).map(cat => (
            <button
              ...
              className={`px-4 sm:px-6 py-2.5 sm:py-3 rounded-full text-xs sm:text-sm font-sans tracking-wide transition-all duration-300 relative ${
                activeCategory === cat
          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.length > 0 ? filteredProjects.map((project, idx) => (
              ...
              >
                ...
                <span className="absolute top-6 left-6 z-30 px-3.5 py-1.5 rounded-full bg-slate-900/85 backdrop-blur-md text-white font-sans text-[11px] font-medium tracking-widest uppercase flex items-center gap-1.5 shadow-md">
                ...
                <div className="p-5 sm:p-8 flex-1 flex flex-col justify-between bg-white relative">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500 font-sans line-clamp-2 leading-relaxed">
                      {project.description}
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-4">
                    {project.tags.slice(0, 3).map(tag => (
                      <span
                        key={tag}
                        className="text-[11px] font-sans font-medium text-slate-500 bg-slate-50 px-2.5 py-1 rounded-md border border-slate-100"
                      >
                        #{tag}
                      </span>
                    ))}
                    {project.tags.length > 3 && (
                      <span className="text-[11px] font-sans text-rose-500 bg-rose-50 px-2 py-1 rounded-md self-center">
              ...
              >
                ...
                <p className="text-slate-500 font-sans text-sm">No projects published yet.</p>
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
              ...
              <div className="w-full lg:w-1/2 p-5 sm:p-8 lg:p-12 overflow-y-auto max-h-none lg:max-h-[800px] flex flex-col justify-between">
                <div>
                  <div className="hidden lg:block mb-6">
                    <span className="px-3.5 py-1.5 rounded-full bg-slate-900 text-white font-sans text-xs tracking-wider uppercase font-semibold">
                  ...
                  {selectedProject.fashionDetails && (
                    <div className="mb-8 p-5 bg-rose-50/50 rounded-2xl border border-rose-100/60">
                      ...
                      <div className="space-y-2.5 text-xs font-sans text-slate-700">
                        <p>
                      ...
                      <div className="space-y-2.5 text-xs font-sans text-slate-700">
                        ...
                        <div className="pt-1.5 flex flex-wrap gap-1.5">
                          {selectedProject.graphicDetails.software?.map(soft => (
                            <span
                              key={soft}
                              className="px-2 py-0.5 rounded bg-emerald-100 text-[10px] font-sans font-medium text-emerald-800"
                            >
                              {soft}
                            </span>
                          ))}
                  ...
                  <div className="mb-4">
                    <span className="text-xs font-sans font-bold text-slate-800 tracking-wider uppercase block mb-2.5">
                      TAG CREDITS
                    </span>
                    <div className="flex flex-wrap gap-1.5">
                      {selectedProject.tags.map(t => (
                        <span
                          key={t}
                          className="px-3 py-1 bg-slate-50 border border-slate-100 text-slate-500 text-xs font-sans rounded-md"
                        >
                          #{t}
                        </span>
                      ))}
                ...
                <div className="mt-8 pt-6 border-t border-slate-100 flex flex-col sm:flex-row gap-3">
                  <button
                    ...
                    className="flex-1 py-3 px-3 bg-rose-600 hover:bg-rose-700 font-sans font-medium text-white rounded-xl text-center text-xs sm:text-sm transition-all duration-300 flex items-center justify-center gap-2 hover:translate-x-1"
                  >
                  ...
                  <button
                    ...
                    onClick={() => setSelectedProject(null)}
                    className="py-3 px-6 bg-slate-50 hover:bg-slate-100 font-sans text-slate-600 rounded-xl text-center text-sm transition-all"
                  >
                    Keep Browsing
                  </button>
                </div>
